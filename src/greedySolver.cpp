...
GreedySolver::GreedySolver(Instance* inst) : Solver::Solver(inst) {
  ...
  logn2("GreedySolver::GreedySolver BEGIN" + name + ": " + desc + " inst: " +
        inst->name);
  ...
  stations_triees =
      std::vector<Station*>(inst->stations->begin(), inst->stations->end());
  ...
  // Code pour avoir un vecteur de stations trié selon les déficits:
  remorques_triees =
      std::vector<Remorque*>(inst->remorques->begin(), inst->remorques->end());
  std::sort(remorques_triees.begin(), remorques_triees.end(),
            [](Remorque* r1, Remorque* r2) { return r1->capa > r2->capa; });

  logn2("GreedySolver::GreedySolver END construit inst: " + inst->name);
}

GreedySolver::~GreedySolver() { delete this->solution; }
...
bool GreedySolver::solve() {
  // for (std::vector<Station*>::iterator i = stations_triees.begin();
  //      i != stations_triees.end(); i++)
  ...
  // Creation d'une nouvelle solution pour ne pas modifier la principale si
  // echec
  ...
  int stations_to_fill_in_first =
      inst->stations->size() - inst->remorques->size() + 1;

  for (int i = 0;
       i < (stations_to_fill_in_first - stations_to_fill_in_first % 2) / 2;
       i++) {
    // on insere la station la plus déficitaire dans la premiere remorque
    logn2("GreedySolver::solve: ajout de la station " +
          stations_triees.at(i)->name + " à la remorque " +
          circuit->remorque->name);
    ...
    if (circuit->desequilibre != 0) {
      ...
      circuit->insert_best(
          stations_triees.at(i));  // on l'insere au bon endroit
      circuit->update();
      ...
      if (circuit->desequilibre != 0) {
        ...
        while ((it != circuit->stations->end()) &&
               ((*it)->id != stations_triees.at(i)->id))
          it++;
    ...
    ++begin_solo;
    // push pour le max
    logn2("GreedySolver::solve: charge de" +
          std::to_string(circuit->charges->at(stations_triees.at(i))) +
          "apres la station " + stations_triees.at(i)->name);
    ...
    circuit->stations->push_back(
        stations_triees.at(stations_triees.size() - 1 - i));
    // on teste le desequi
    circuit->update();
    if (circuit->desequilibre != 0) {
      circuit->stations->pop_back();  // supprime la déreniere station
      ...
      if (circuit->desequilibre != 0) {
        ...
        while ((it != circuit->stations->end()) &&
               ((*it)->id !=
                stations_triees.at(stations_triees.size() - 1 - i)->id))
          it++;
        ...
        circuit->update();
        // on l'insert dans la deuxiéme remorque
        sol->circuits->at(remorques_triees.at(1)->id)
    ...
    logn2("GreedySolver::solve: charge de" +
          std::to_string(
              (circuit->charges)
                  ->at(stations_triees.at(stations_triees.size() - 1 - i))) +
          "apres la station " +
          stations_triees.at(stations_triees.size() - 1 - i)->name);
  }
  if (stations_to_fill_in_first % 2 == 1) {
    ...
    if (circuit->desequilibre != 0) {
      auto it = circuit->stations->begin();
      while ((it != circuit->stations->end()) &&
             ((*it)->id != stations_triees.at(begin_solo)->id))
        it++;
  ...
  for (int circuit_id = 0; circuit_id < inst->remorques->size(); ++circuit_id) {
    if (circuit_id != remorques_triees.at(0)->id) {
      ...
      logn5("GreedySolver::solve: ajout de la station " +
            stations_triees.at(begin_solo)->name + " à la remorque " +
            circuit->remorque->name);
      circuit->stations->push_back(stations_triees.at(begin_solo));
      ++begin_solo;
    }
