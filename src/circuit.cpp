...
void Circuit::equilibrate() {
    ...
    int sum_depots=0;
    int sum_charges=this->charge_init;
    for (auto it = this->stations->begin(); it != this->stations->end(); ++it) {
        ...
        sum_depots+=station->deficit();
        }
        else if((station->deficit()==0)
        {
        (*this->depots)[station]=0;
        }
      ...
      (*this->charges)[station] = this->charge_init - sum_depots;}
      //le cas ou il ya un plus de velo dans la stations donc la charge de remorque va augmanter
      if (station->deficit()<0) && (-(station->deficit()+sum_charges-sum_depots <= remorque->capa)
      {
        sum_charges+= -(station->deficit());

       (*this->charges)[station] = this->charge_init +sum_charges -sum_depots;}
       else if (-(station->deficit()+this->charges_init -sum_depots > remorque->capa)
       {sum_charges+=-station->deficit()-(remorque->capa-(this->charges_init -sum_depots));


       (*this->charges)[station] = remorque->capa ;}
        ...
        if (station->deficit()>=0)
        {
        this->desequilibre += abs(station->deficit())-sum_depots;
        }
        else if (station->deficit()<0)
        {
         this->desequilibre += abs(station->deficit())-sum_charges+sum_depots;
        }
    }
    // Calcul savant de la charge initiale de la remorque pour garantir les
    // dépots et retraits qui viennent d'ê
